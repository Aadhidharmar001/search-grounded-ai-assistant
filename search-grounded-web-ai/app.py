    ...
    def decorated_function(*args, **kwargs):
        ...
        if len(request_counts[client_ip]) >= RATE_LIMIT:
            return render_template("index.html",
                                 error="Rate limit exceeded. Please wait before making another request."), 429
...
def parse_answer_with_citations(answer, sources):
    ...
    Returns: (parsed_answer, citation_map)
    """
    import re
    ...
    # Keep citations as plain text [1], [2], etc. - no HTML generation
    ...
    if request.method == "POST":
        try:
            ...
            if not question:
                ...
                error = "Please ask a more complete question."
            else:
...
def health_check():
    ...
    return {"status": "healthy", "timestamp": "2026-02-16T00:00:00Z"}
